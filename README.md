...
团队不再是右下角的悬浮按钮，而是**会话视图环里的第三个页签**：`对话 / 轨迹 / Agent 团队`（`src/client/`）。点开是一间**能看见的办公室**，不是一张列表。
...
- **屏幕上有东西**：工作位的显示器不是装饰——上面写着这名成员**正在做的那件事**：它名下进行中的任务标题，没有就是最近一条发给它的消息。没活干的屏幕是暗的。
...
- **消息是气泡，不是行**：主会话发出的靠右、队友发来的靠左，连续同向自动收起头像；`汇报` 走成功色左边条，`已收工` 是虚线弱化，第 3 跳起的 `hop` 徽标变警告色。鼠标停在气泡上，对应成员的工位会亮起——**同一份状态的两种投影互相指认**。
- **共享工作区自成一块**：黑板上的每条笔记一张卡（名字、首行预览、作者、时间），鼠标停上去同样会点亮作者；标题右边写着这份快照的时间。
- **任务是看板**：待办 / 进行中 / 已完成三列，卡片带指派人与结案备注；成员身上还挂着它名下未完成任务的计数徽标。
- 点任何一头鲸开对应 transcript（走 durable 的 subagent 地址，目录没拉取过就刷新后重试），点 leader 回主会话，`aria-current` 标出你正在看的那个。
- 全部动效在 `prefers-reduced-motion` 下关闭（信使直接停在终点）。
...
pnpm run test        # 174 个单测：折叠 / 投影 / 服务授权矩阵 / 工具契约 / 队友组装 / 会话预算 / 虚拟工作区 / 房间几何 / 客户端跟随与页签 / 协作室
