...
- **每张桌子都是一整套**：桌面 + 键盘 + 该席位强调色的马克杯 + 一沓纸，桌上立着**显示器**（加大号的屏幕、支架、底座），桌前一把**办公椅**（椅面、立柱、五爪底座）。全部是 token 画出来的形状，没有一张贴图。
...
- **人是戴海兽头套的人**：一具人形（鞋、裤子、衬衫、两条胳膊、门襟纽扣/肩缝）+ 一头**侧面**的鲸鱼或鲨鱼当兜帽——只有侧着才像它。leader 戴蓝鲸（喷水），队友按席位轮流拿到虎鲸（背鳍 + 白眼斑）、座头鲸（长鳍 + 头瘤）、独角鲸（长牙）、白鲸（额隆）、抹香鲸（方头）、鲨鱼（背鳍 + 鳃裂）。正面有眉、眼高光、腮红和微笑，背面有头发高光；同一具身体，差别只在头套和细节上。强调色是**品牌 token 旋转出来的**（`hue-rotate`），不写死颜色。背身时头套朝另一边、后脑勺有头发、腿收进椅子里。
...
- **不标区域，靠家具说话**：后墙上有透着海水的舷窗、白板和挂钟，右手边是休息角的地毯与沙发。房间里没有一个"工作区""休息区"的字牌——看得出来就不用写。
