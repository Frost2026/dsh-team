window.__ModuleLoader__.load({
  ...
  `;
		...
		function observeSubagentsMenu(store, sessions) {
			...
			const processMenu = () => {
				const subagentMenu = Array.from(document.querySelectorAll("[role=\"tree\"]")).find((m) => m.querySelector("[data-state]") !== null);
				...
				const activeRows = [];
				const dismissedRows = [];
				if (activeMemberIds.size > 0 || dismissedMemberIds.size > 0) for (const row of rows) {
					const sessionId = getRowSessionId(row);
					if (!sessionId) continue;
					if (dismissedMemberIds.has(sessionId)) dismissedRows.push(row);
					else if (activeMemberIds.has(sessionId)) activeRows.push(row);
					else if (activeMemberIds.size > 0) dismissedRows.push(row);
