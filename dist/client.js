	...
	factory: (require) => {
		...
		*/
		...
		function TeammateModelBadge({ sessionId, store, sessions }) {
			...
			let member = teamState.members.find((m) => m.memberId === currentId);
			let isDismissed = false;
			if (!member && teamState.dismissedMembers) {
				const dismissed = teamState.dismissedMembers.find((m) => m.memberId === currentId);
				if (dismissed) {
					member = dismissed;
					isDismissed = true;
				}
			...
			if (!member && summary?.parentId) {
				...
				if (parentBinding) {
					...
					if (parentTeam) {
						member = parentTeam.members.find((m) => m.memberId === currentId);
						if (!member && parentTeam.dismissedMembers) {
							const dismissed = parentTeam.dismissedMembers.find((m) => m.memberId === currentId);
							if (dismissed) {
								member = dismissed;
								isDismissed = true;
							}
		...
		*/
		...
		function panelState(leaderId, currentId, team) {
			return {
				...
				...team.dismissedMembers !== void 0 ? { dismissedMembers: team.dismissedMembers } : {},
				tasks: team.tasks,
  ...
  `;
		...
		function getRowSessionId(row) {
			...
			while (curr) {
				...
				if (curr.key && typeof curr.key === "string" && curr.key.length > 20 && curr.key.includes("-")) return curr.key;
				curr = curr.return;
			}
			return null;
		}
		function observeSubagentsMenu(store, sessions) {
			if (typeof document === "undefined") return;
			ensureDismissedStyles();
			const processMenu = () => {
				const subagentMenu = Array.from(document.querySelectorAll("[role=\"tree\"]")).find((m) => {
					const cls = m.className || "";
					return (cls.includes("menu") || cls.includes("Menu") || m.querySelector("[data-state]") !== null) && m.innerText.includes("continuable");
				});
				if (!subagentMenu) return;
				const rows = Array.from(subagentMenu.querySelectorAll(":scope > div, :scope > [role=\"treeitem\"]")).filter((el) => el.getAttribute("role") === "treeitem" || el.querySelector("[role=\"treeitem\"]") !== null);
				if (rows.length === 0) return;
				let activeMemberIds = null;
				const state = store.getSnapshot();
				if (state.members.length > 0) activeMemberIds = new Set(state.members.map((m) => m.memberId));
				else try {
					...
					const parentId = currentSession ? summaries[currentSession]?.parentId : void 0;
					if (parentId) {
						const parentFace = sessions.binding?.(parentId)?.session?.projections?.faceOf("team")?.getSnapshot();
						if (parentFace?.members && parentFace.members.length > 0) activeMemberIds = new Set(parentFace.members.map((m) => m.memberId));
					}
				} catch {}
				const activeRows = [];
				const dismissedRows = [];
				if (activeMemberIds && activeMemberIds.size > 0) for (const row of rows) {
					const sessionId = getRowSessionId(row);
					if (!sessionId) continue;
					if (activeMemberIds.has(sessionId)) activeRows.push(row);
					else dismissedRows.push(row);
				}
				else {
					const roleGroups = /* @__PURE__ */ new Map();
					for (const row of rows) {
						const roleName = (row.querySelector("span[class*=\"label\"]")?.innerText || row.innerText || "").split("\n")[0].replace(/已解散/g, "").trim().split(" ")[0].toLowerCase();
						if (!roleName) continue;
						if (!roleGroups.has(roleName)) roleGroups.set(roleName, []);
						roleGroups.get(roleName).push(row);
					}
					for (const list of roleGroups.values()) if (list.length > 1) {
						...
						for (let i = 0; i < list.length; i++) if (i === activeIdx) activeRows.push(list[i]);
						else dismissedRows.push(list[i]);
					} else activeRows.push(list[0]);
				}
				for (const row of activeRows) {
					if (row.hasAttribute("data-dsh-dismissed")) row.removeAttribute("data-dsh-dismissed");
					row.querySelectorAll(".dsh-dismissed-tag").forEach((tag) => tag.remove());
				}
				for (const row of dismissedRows) {
					if (row.getAttribute("data-dsh-dismissed") !== "true") row.setAttribute("data-dsh-dismissed", "true");
					const label = row.querySelector("span[class*=\"label\"]");
			...
			new MutationObserver(processMenu).observe(document.body, {
				childList: true,
				subtree: true
			});
		}
