	...
	factory: (require) => {
		...
		*/
		...
		function TeammateModelBadge({ sessionId, store, sessions }) {
			...
			const isSubagent = summary?.origin === "subagent" || sessions.subagentAddress(currentId) !== void 0;
		...
		*/
		...
		function observeSubagentsMenu(store) {
			...
			const processMenu = () => {
				...
				const activeNames = new Set(state.members.map((m) => m.name.toLowerCase()));
				const validRows = Array.from(subagentMenu.children).filter((c) => c.getAttribute("role") === "treeitem" || c.querySelector("[role=\"treeitem\"]") !== null);
				...
				const dismissedRows = [];
				for (const [name, rows] of seenNames.entries()) if (rows.length > 1) {
					const activeOne = rows[rows.length - 1];
					activeOne.dataset.dshActive = "true";
					const older = rows.slice(0, -1);
					dismissedRows.push(...older);
				} else if (!activeNames.has(name) && state.members.length > 0) dismissedRows.push(rows[0]);
