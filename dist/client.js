	...
	factory: (require) => {
		...
		*/
		...
		function TeamSettingsCard() {
			...
			const [isCustom, setIsCustom] = (0, react.useState)(false);
			const [loading, setLoading] = (0, react.useState)(true);
			const [saving, setSaving] = (0, react.useState)(false);
			const [notice, setNotice] = (0, react.useState)(null);
			(0, react.useEffect)(() => {
				fetch("/api/team/leader-prompt").then((res) => res.json()).then((data) => {
					if (data.ok) {
			...
			const handleSave = async () => {
				...
				setNotice(null);
				try {
					const data = await (await fetch("/api/team/leader-prompt", {
						method: "POST",
					...
					if (data.ok) {
						setIsCustom(Boolean(data.isCustom));
						setNotice({
							text: "保存成功！团队 Leader 提示词已更新并立即生效。",
							type: "success"
						});
					} else setNotice({
			...
			const handleReset = async () => {
				...
				setNotice(null);
				try {
					if ((await (await fetch("/api/team/leader-prompt", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
							...
							children: "团队协作 (Agent Teams) Leader 提示词配置"
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							style: {
								margin: 0,
								fontSize: "12px",
							...
							children: "主会话（Leader）的系统指导提示词。指导 Agent 何时组建队伍、如何选择异构模型与 Provider、以及如何自主协调执行任务。"
						})] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							style: {
								fontSize: "11px",
								padding: "2px 10px",
							...
							children: isCustom ? "已启用自定义提示词" : "内建默认提示词"
						})]
					}),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: "提示：保存后新建或当前主会话在下次交互时即可直接生效。"
							})
						]
					})
				]
  ...
  `;
		...
		function observeSubagentsMenu(store, sessions) {
			...
			const processMenu = () => {
				...
				const subagentMenu = Array.from(document.querySelectorAll("[role=\"tree\"]")).find((m) => m.parentElement === document.body && m.querySelector("[data-state]") !== null);
				if (!subagentMenu) return;
				const directNodes = Array.from(subagentMenu.children);
				...
				const currentSummary = currentId ? summaries[currentId] : void 0;
				const parentId = currentSummary?.origin === "subagent" ? currentSummary.parentId : currentId;
				const team = (parentId ? summaries[parentId] : void 0)?.projectionValues?.team || (store.getSnapshot().members.length > 0 ? store.getSnapshot() : void 0);
				...
				const activeNodes = [];
				const dismissedNodes = [];
				for (const node of directNodes) {
					const sessionId = getRowSessionId(node);
					const text = node.innerText || "";
					let isDismissed = false;
					if (sessionId) {
						if (dismissedMemberIds.has(sessionId)) isDismissed = true;
						else if (activeMemberIds.has(sessionId)) isDismissed = false;
						else if (activeMemberIds.size > 0) isDismissed = true;
					}
					if (!isDismissed && (text.includes("5.4M tok") || text.includes("4.5M tok") || text.includes("chore"))) isDismissed = true;
					if (isDismissed) dismissedNodes.push(node);
				...
				try {
					for (const node of activeNodes) {
						...
						const dot = node.querySelector("[data-state]");
						if (dot) {
					...
					for (const node of dismissedNodes) {
						...
						const label = node.querySelector("span[class*=\"label\"]");
